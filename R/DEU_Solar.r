...
for ( UntilDay in c(150, 180, 210, 240, 365) ) {
  ...
  ra = lm ( data = DT %>% filter( Jahr == J & Tag <= UntilDay ), formula = CumSolar  ~ sin(TagRad) )
...
ggsave(  file = paste( outdir, 'DEU_Solar.png', sep = '')
               , plot = p
               , bg = "white"
               , width = 1920
               , height = 1080
